    ...
    def extract_file_path(file_info: Union[str, Dict]) -> Optional[str]:
        ...
        try:
            ...
            if isinstance(file_info, str):
                ...
                if file_info.endswith(('.md', '.pdf', '.txt', '.docx', '.doc', '.html', '.htm')):
                ...
                try:
                    ...
                    if not info_dict:
                        ...
                        raise ValueError(f"Input is neither a valid file path nor JSON: {file_info}")
            else:
                info_dict = file_info
            ...
            if isinstance(file_input, str):
                ...
                try:
                    ...
                    if isinstance(parsed_json, dict) and 'paper_path' in parsed_json:
                        ...
                        if file_path and not os.path.exists(file_path):
                            ...
                            if os.path.isdir(paper_dir):
                                ...
                                if not file_path:
                                    raise ValueError(f"No markdown file found in directory: {paper_dir}")
                    else:
                        raise ValueError(f"Invalid JSON format: missing paper_path")
                except json.JSONDecodeError:
                    ...
                    if extracted_json and 'paper_path' in extracted_json:
                        ...
                        # 不是JSON，按文件路径处理
                        # Check if it's a file path (existing or not)
                        if file_input.endswith(('.md', '.pdf', '.txt', '.docx', '.doc', '.html', '.htm')):
                            ...
                            if os.path.isfile(file_input):
                                ...
                                if not file_path:
                                    raise ValueError(f"No markdown file found in directory: {file_input}")
                        else:
