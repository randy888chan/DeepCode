    ...
    def convert_to_markdown(self, input_file: str, output_file: Optional[str] = None, extract_images: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.is_url(input_file):
                ...
                # 对于URL，检查是否为支持的格式
                if not input_file.lower().endswith(('.pdf', '.docx', '.pptx', '.html', '.md', '.txt')):
                    return {
                        "success": False,
...
async def download_files(instruction: str) -> str:
                ...
                if not conversion_success and DOCLING_AVAILABLE:
                    try:
                        ...
                        if converter.is_supported_format(destination):
                            conversion_result = converter.convert_to_markdown(destination, extract_images=True)
                            if conversion_result["success"]:
                                ...
                                if conversion_result.get('images_extracted', 0) > 0:
                                    ...
                                    images_dir = os.path.join(os.path.dirname(conversion_result['output_file']), 'images')
    ...
    for local_path in local_paths:
        try:
            ...
            # 检查目标文件是否已存在
            if os.path.exists(destination):
                results.append(f"[WARNING] Skipped {local_path}: File already exists at {destination}")
                continue
        ...
        if not conversion_success and DOCLING_AVAILABLE:
            try:
                ...
                if converter.is_supported_format(target_path):
                    conversion_result = converter.convert_to_markdown(target_path, extract_images=True)
                    if conversion_result["success"]:
                        msg += f"\n\n[INFO] Document converted to Markdown (docling)"
                        msg += f"\n   Markdown file: {conversion_result['output_file']}"
                        msg += f"\n   Conversion time: {conversion_result['duration']:.2f} seconds"
...
@mcp.tool()
async def move_file_to(source: str, destination: Optional[str] = None, filename: Optional[str] = None) -> str:
    """
    ...
    if result["success"]:
        msg += f"[SUCCESS] File moved successfully!\n"
        msg += f"   From: {source}\n"
        ...
        msg += f"[ERROR] Move failed!\n"
        msg += f"   Error: {result['error']}"
    
    return msg

@mcp.tool()
async def convert_document_to_markdown(file_path: str, output_path: Optional[str] = None, extract_images: bool = True) -> str:
    """
    ...
    if not is_url_input and file_path.lower().endswith('.pdf') and PYPDF2_AVAILABLE and not extract_images:
        try:
            simple_converter = SimplePdfConverter()
        ...
        try:
            ...
            elif is_url_input and not file_path.lower().endswith(('.pdf', '.docx', '.pptx', '.html', '.md', '.txt')):
                return f"[ERROR] Unsupported URL format: {file_path}"
            
            # 执行转换（支持图片提取）
            result = converter.convert_to_markdown(file_path, output_path, extract_images)
        except Exception as e:
    ...
    if result["success"]:
        ...
        if extract_images and 'images_extracted' in result:
            ...
            if images_count > 0:
                ...
                msg += f"   No images found in document\n"
...
@mcp.tool()
async def convert_url_to_markdown(url: str, output_path: Optional[str] = None, extract_images: bool = True) -> str:
    """
    ...
    try:
        ...
        if result["success"]:
            msg = f"[SUCCESS] Document converted from URL successfully!\n"
            ...
            if extract_images and 'images_extracted' in result:
                ...
                if images_count > 0:
                    msg += f"   Images extracted: {images_count}\n"
                    images_dir = os.path.join(os.path.dirname(result['output_file']), 'images')
...
if __name__ == "__main__":
    ...
    print("\nAvailable tools:")
    print("  • download_files - Download files or move local files from natural language instructions (auto-converts to MD)")
    print("  • download_and_convert - Download/move files with explicit conversion control")
    print("  • convert_document_to_markdown - Convert existing documents to Markdown with image extraction")
    print("  • convert_url_to_markdown - Convert documents from URL directly to Markdown (no download)")
    print("  • parse_download_urls - Extract URLs, local paths and destination paths without action")
