        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if match.startswith('git@'):
                    ...
                    if '/' in match and not any(x in match for x in ['./', '../', 'agent_folders', 'tools']):
                        parts = match.split('/')
                        if (len(parts) == 2 and 
                            all(part.replace('-', '').replace('_', '').isalnum() for part in parts) and
    ...
    for url in urls:
        try:
            ...
            if target_path:
                ...
                if os.path.isabs(target_path):
                    ...
                    if os.path.basename(target_path) == '' or target_path.endswith('/'):
                        final_path = os.path.join(target_path, extractor.infer_repo_name(url))
                else:
                    # 如果是相对路径，保持相对路径
                    final_path = target_path
                    # 如果目标路径是目录，添加仓库名
            ...
            if os.path.exists(final_path):
                results.append(f"❌ Failed to download {url}: Target path already exists: {final_path}")
                continue
...
@mcp.tool() 
async def git_clone(repo_url: str, target_path: Optional[str] = None, branch: Optional[str] = None) -> str:
    """
