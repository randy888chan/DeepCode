...
4. **Next Steps**: List code files that will be implemented in the next round
...
- Predict next implementation steps based on the initial plan
...
"""
    ...
    def _create_fallback_code_summary(self, file_path: str, implementation_content: str, files_implemented: int) -> str:
        ...
        Returns:
            Fallback summary
        """
        current_phase = self._determine_file_phase(file_path)
        current_round = self.current_round
        
        # Extract basic function info
        functions = self._parse_functions_from_content(implementation_content)
...
1. **Status Marker**: Phase {{}}, Round {current_round}

2. **Implementation Progress**: {file_path}: Basic implementation completed

3. **Dependencies**: According to the File Structure and initial plan, list the required parameters, 
return parameters and core ideas of each function in the implemented file that may be called by other code files in the future. 
(format as {file_path}: Function {{}}: core ideas--{{}}; Required parameters--{{}}; Return parameters--{{}}
Required packages: {{}})

4. **Next Steps**: Code will be implemented: [Next file from plan]; will stay on Phase {{}}
...
**Next Action:** Based on the implementation plan:
1. Use read_code_mem to check for existing file summaries (if available)
2. Use read_file to understand dependencies and context (if available)
3. Use get_file_structure to understand project layout if needed
4. Use search_code to find code patterns in the project
5. Use search_reference_code for similar implementations if needed
6. Use execute_python or execute_bash for testing/validation if needed
7. Then implement the next required code file using write_file

**Available Essential Tools:** read_code_mem, read_file, write_file, execute_python, execute_bash, search_code, search_reference_code, get_file_structure

**Remember:** Focus on implementing ONE complete file per round."""
            }
