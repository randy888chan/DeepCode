...
1. **➡️ FIRST: Call read_code_mem(already_implemented_file_path)** to understand existing implementations and dependencies - agent should choose relevant ALREADY IMPLEMENTED file paths for reference, NOT the new file you want to create
2. Then: Use write_file to implement the new component
...
**Remember:** ALWAYS start with read_code_mem to query summaries of already-implemented files (agent chooses which files to reference, NOT the new file being created) to understand current implementations. Implement according to the original paper's specifications - any reference code is for inspiration only."""
