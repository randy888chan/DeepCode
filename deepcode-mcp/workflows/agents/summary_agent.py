...
class SummaryAgent:
    ...
    async def generate_conversation_summary(
        ...
        try:
            self.logger.info(
                "Generating role-aware conversation summary using Summary Agent"
            )

            # Prepare role-aware summary request / 准备角色感知的总结请求
            recent_messages = (
                messages[-25:] if len(messages) > 25 else messages
            )  # 稍微增加上下文
            ...
            summary_messages = [
                ...
                {
                    "role": "user",
                    "content": f"""ROLE DISTRIBUTION ANALYSIS:
...
Please provide a role-aware summary that maintains clear distinction between user guidance and assistant responses.""",
            ...
            if not self._validate_role_aware_summary(summary_content):
                self.logger.warning(
                    "Generated summary lacks role clarity, enhancing..."
                )
            ...
            self.logger.info(
                f"Role-aware summary generated successfully, length: {len(summary_content)} characters"
            )
            return summary_content

        except Exception as e:
            self.logger.error(
                f"Failed to generate role-aware conversation summary: {e}"
            )
    ...
    async def _call_llm_for_summary(
        ...
        if client_type == "anthropic":
            ...
            openai_messages = [
                {
                    "role": "system",
                    "content": "You are a helpful conversation summarization assistant.",
                }
            ]
    ...
    def _prepare_messages_for_summary(self, messages: List[Dict]) -> List[Dict]:
        ...
        for i, msg in enumerate(messages):
            ...
            if role == "unknown":
                ...
                if any(
                    pattern in content
                    for pattern in ["Tool Result", "[", "Error detected", "继续"]
                ):
                ...
                self.logger.debug(
                    f"Inferred role for message {i}: {cleaned_msg['role']}"
                )
            ...
            if len(content) > 800:  # Truncate very long messages
                ...
                self.logger.debug(
                    f"Truncated long message {i} from {len(content)} to {len(cleaned_msg['content'])} chars"
                )
    ...
    def _validate_role_aware_summary(self, summary: str) -> bool:
        ...
        found_indicators = sum(
            1 for indicator in role_indicators if indicator in summary_lower
        )
        ...
        found_sections = sum(
            1 for section in structured_sections if section.lower() in summary_lower
        )

        is_valid = found_indicators >= 2 and found_sections >= 2
        self.logger.debug(
            f"Summary validation: {found_indicators} role indicators, {found_sections} sections, valid: {is_valid}"
        )
    ...
    def _enhance_summary_with_role_context(
        ...
        completed_files = [
            f.get("file", "unknown")
            for f in implementation_summary.get("completed_files", [])
        ]
...
**Next Steps:** Continue with next file implementation according to plan priorities."""
    ...
    def apply_sliding_window(
        ...
        try:
            self.logger.info(
                f"Applying sliding window mechanism with role-aware processing, window_size: {window_size}"
            )
            ...
            self.logger.info(
                f"Before sliding window - Role distribution: {role_analysis_before['role_counts']}"
            )
            self.logger.info(
                f"Before sliding window - Last 10 roles: {[msg.get('role') for msg in messages[-10:]]}"
            )
            ...
            if initial_plan_message:
                ...
                if initial_plan_message.get("role") != "user":
                    self.logger.warning(
                        f"Initial plan message has unexpected role: {initial_plan_message.get('role')}, forcing to 'user'"
                    )
...
I will now continue implementing the remaining files according to the original plan. Please provide next guidance or tool results.""",
            ...
            self.logger.info(
                f"Role-aware summary message added as 'assistant' role, summary length: {len(summary)} characters"
            )
            ...
            if len(messages) > recent_cutoff:
                ...
                if filtered_recent:
                    # 验证并纠正角色序列
                    validated_recent_messages = self._validate_role_sequences(
                        filtered_recent
                    )
                    new_messages.extend(validated_recent_messages)

                    self.logger.info(
                        f"Added only {len(validated_recent_messages)} most recent messages (cutoff={recent_cutoff}) to avoid duplicating summarized content"
                    )
                else:
                    self.logger.info(
                        "No recent messages to add after filtering out initial plan"
                    )
            else:
                self.logger.info(
                    f"Message count ({len(messages)}) <= cutoff ({recent_cutoff}), no additional recent messages added"
                )
            ...
            if new_messages:
                ...
                if last_role != "user":
                    ...
                    self.logger.info(
                        "Added continuation message to ensure proper conversation flow"
                    )
            ...
            self.logger.info(
                f"After sliding window - Role distribution: {role_analysis_after['role_counts']}"
            )
            self.logger.info(
                f"After sliding window - Final message roles: {[msg.get('role') for msg in new_messages]}"
            )

            # Verify conversation continuity / 验证对话连续性
            if (
                role_analysis_after["role_counts"].get("user", 0) == 0
                or role_analysis_after["role_counts"].get("assistant", 0) == 0
            ):
                self.logger.warning(
                    "⚠️  Role imbalance detected after sliding window - missing user or assistant messages!"
                )

            self.logger.info(
                f"Sliding window applied with role awareness: {len(messages)} -> {len(new_messages)} messages"
            )
            ...
            self.logger.error(
                f"Failed to apply sliding window with role awareness: {e}"
            )
    ...
    def _detect_role_issues(self, role_sequence: List[str]) -> List[str]:
        ...
        for i in range(len(role_sequence) - 1):
            if role_sequence[i] == role_sequence[i + 1] and role_sequence[i] in [
                "user",
                "assistant",
            ]:
                issues.append(
                    f"Consecutive {role_sequence[i]} roles at positions {i}-{i+1}"
                )
    ...
    def _extract_recent_complete_rounds(
            ...
            if role == "assistant" and len(current_round) >= 2:
                ...
                if prev_role == "user":
                    ...
                    for round_msg in current_round:
                        ...
                        if (
                            round_msg.get("role") == "assistant"
                            and len(temp_round) >= 2
                        ):
                    ...
                    if user_assistant_pairs:
                        ...
                        current_round = (
                            temp_round  # Keep remaining messages for next round
                        )
        ...
        if current_round:
            ...
            if valid_remaining:
                ...
                self.logger.info(
                    f"Added incomplete round with {len(valid_remaining)} messages"
                )
        ...
        self.logger.info(
            f"Extracted {len(recent_rounds)} complete rounds containing {len(recent_messages)} messages"
        )
        self.logger.info(
            f"Final role sequence: {[msg.get('role') for msg in recent_messages[-10:]]}"
        )
    ...
    def _ensure_role_alternation(self, messages: List[Dict]) -> List[Dict]:
        ...
        for i, msg in enumerate(messages):
            ...
            if current_role == last_role and last_role in ["user", "assistant"]:
                ...
                if len(content) > len(clean_messages[-1].get("content", "")):
                    ...
                    self.logger.debug(
                        f"Replaced duplicate {current_role} role at position {i} with more informative content"
                    )
                else:
                    self.logger.debug(
                        f"Skipping duplicate {current_role} role at position {i}"
                    )
                continue
        ...
        if clean_messages:
            ...
            if final_role != "user":
                ...
                self.logger.info(
                    "Added continuation message to ensure proper role ending"
                )
        ...
        for i, msg in enumerate(messages):
            ...
            if original_role == "unknown" or original_role not in ["user", "assistant"]:
                ...
                if (
                    ...
                    or "CONTINUE" in content.upper()
                ):
                    validated_msg["role"] = "user"
                    self.logger.warning(
                        f"Message {i}: role corrected from '{original_role}' to 'user' based on content"
                    )
                else:
                    # Default to alternating pattern / 默认使用交替模式
                    expected_role = (
                        "assistant" if len(validated_messages) % 2 == 0 else "user"
                    )
                    validated_msg["role"] = expected_role
                    self.logger.warning(
                        f"Message {i}: role corrected from '{original_role}' to '{expected_role}' based on position"
                    )

            # Ensure content is not empty / 确保内容不为空
            if not validated_msg.get("content", "").strip():
                self.logger.warning(
                    f"Message {i}: empty content detected, adding placeholder"
                )
    ...
    def _log_final_role_structure(self, messages: List[Dict]):
        ...
        for i, msg in enumerate(messages):
            ...
            self.logger.info(
                f"  [{i:2d}] {role:9s} | {content_type:12s} | {content_preview}"
            )
    ...
    def _emergency_message_trim(
        self, messages: List[Dict], initial_plan_message: Optional[Dict]
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            self.logger.warning(
                f"Emergency trim applied: {len(messages)} -> {len(new_messages)} messages"
            )
            return new_messages
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "latest_summary_time": self.summary_history[-1]["timestamp"]
            if self.summary_history
            else None,
            "average_summary_length": sum(
                len(s["summary"]) for s in self.summary_history
            )
            / len(self.summary_history)
            if self.summary_history
            else 0,
        }
    ...
    def analyze_message_patterns(self, messages: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            if analysis["message_lengths"]:
                analysis["average_message_length"] = sum(
