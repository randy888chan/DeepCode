        ...
        try:
            ...
            if hasattr(result, 'content'):
                ...
                try:
                    ...
                    result_data = {"status": "success", "file_path": tool_call["input"].get("file_path", "unknown")}
            ...
            if result_data and result_data.get("status") == "success":
                file_path = result_data.get("file_path", tool_call["input"].get("file_path", "unknown"))
            else:
            ...
            if file_path and file_path not in self.implemented_files_set:
                ...
                self.logger.debug(f"File already tracked, skipping duplicate count: {file_path}")
            else:
                # No valid file path found / 没有找到有效的文件路径
                self.logger.warning(f"No valid file path found for tracking")
            ...
            try:
                ...
                if file_path and file_path not in self.implemented_files_set:
                    self.implemented_files_set.add(file_path)
                    self.files_implemented_count += 1
                    self.logger.info(f"File implementation counted (emergency fallback): count={self.files_implemented_count}, file={file_path}")
            except:
                pass
