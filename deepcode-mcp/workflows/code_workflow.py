...
class CodeWorkflow:
    ...
    def _initialize_specialized_agents(self, code_directory: str):
        ...
        self.code_implementation_agent = CodeImplementationAgent(
            mock_mcp_agent, self.logger
        )
...
**Remember:** Always use dependency analysis and file reading before implementation to ensure consistency across all files."""
    ...
    async def _pure_code_implementation_loop(
        self, implementer, code_agent, initial_message: str
    ) -> str:
        ...
        while iteration < max_iterations:
            ...
            try:
                response_content = await implementer.generate_str(
                    ...
                    if messages
                    else "Continue implementing code files..."
                )
                ...
                files_count = (
                    self.code_implementation_agent.get_files_implemented_count()
                    if self.code_implementation_agent
                    else iteration
                )
                ...
                if (
                    self.code_implementation_agent
                    and self.code_implementation_agent.should_trigger_summary(
                        SUMMARY_TRIGGER
                    )
                ):
                    self.logger.info(
                        f"Triggering summary: {files_count} files implemented"
                    )
                ...
                if len(messages) > 120:
                    ...
                    messages = self.summary_agent._emergency_message_trim(
        ...
        try:
            if self.summary_agent:
                                    ...
                                    (),
                                    {
                                        "content": [
                                            type(
                                                ...
                                                {
                                                    "type": "text",
                                                    "text": "Implementation progress summary",
                                                },
                                            )()
                                        ]
                                    },
                ...
                implementation_summary = (
                    self.code_implementation_agent.get_implementation_summary()
                    if self.code_implementation_agent
                    else {}
                )
            ...
            return (
                "Implementation progress summary - continuing with code implementation"
            )
    ...
    async def run_workflow(
        ...
        try:
            ...
            if pure_code_mode:
                ...
                self.logger.info(
                    "Pure code mode is the default and recommended approach"
                )
                results["code_implementation"] = await self.implement_code_pure(
                    plan_content, target_directory
                )
